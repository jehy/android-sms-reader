...
public class ServiceExample extends IntentService {
	...
	private class ThreadDemo extends Thread {
		...
		public void run() {
			...
			/*
			 * long time = GetLastTime(); Log.v("Thread service",
			 * "Searching for messages modified after " + time);
			 * 
			 * CentreonMessage m; if (!s) Log.v("Thread service",
			 * "ReadMessages call failed, aborting"); else while ((m =
			 * messages.getNew(time)) != null) { SmsManager smsManager =
			 * SmsManager.getDefault(); Log.v("Thread service",
			 * "Sending sms to " + m.phone); String message = m.getSmsText();
			 * Log.v("Thread service", "SMS text:  " + message);
			 * 
			 * String phone = m.phone; //
			 * smsManager.sendMultipartTextMessage(phone, null, // msgTexts,
			 * listOfIntents, null); smsManager .sendTextMessage(phone, null,
			 * message, null, null); SetLastTime(m.time); time = m.time;
			 * Log.v("Thread service", "Setting last modification time to " +
			 * m.time); }
			 */}
		 ...
		 * 
		 * final Runtime runtime = Runtime.getRuntime(); if (REMOUNT) {
		 * Log.v("ReadMessages", "Remounting SD card to update contents");
		 * 
		 * String[] str = { "su", "-c", "umount /mnt/sdcard" }; String[] str2 =
		 * { "su", "-c", "mount -r -t vfat /dev/block/vold/179:1 /mnt/sdcard" };
		 * try { runtime.exec(str); runtime.exec(str2); } catch (IOException e2)
		 ...
		 * "/SmsInformer/messages.txt"; PackageManager m = getPackageManager();
		 * String s = getPackageName(); PackageInfo p = null; try { p =
		 * m.getPackageInfo(s, 0); } catch (NameNotFoundException e1) { // TODO
		 * Auto-generated catch block e1.printStackTrace(); } s =
		 * p.applicationInfo.dataDir;
		 * 
		 * String to = s + "/messages.txt"; String command = "cp " + from + " "
		 * + to; Log.v("ReadMessages", command);
		 * 
		 * String[] str3 = { "su", "-c", command }; try { runtime.exec(str3); }
		 * catch (IOException e1) { // TODO Auto-generated catch block
		 * e1.printStackTrace(); }
		 * 
		 * Log.v("ReadMessages", "Reading messages file"); BufferedReader rd;
		 * StringBuilder sb = new StringBuilder(); try { rd = new
		 * BufferedReader(new FileReader(to)); String line = null; // read all
		 * the lines till the end while ((line = rd.readLine()) != null) {
		 * sb.append(line + "\n"); } rd.close(); // close reader if (sb.length()
		 * > 5) { messages.set(sb.toString()); return true; } else {
		 * Log.v("ReadMessages", "Too little messages length, won't parse.");
		 * return false; } } catch (Exception e) { // Log.e("Service run",
